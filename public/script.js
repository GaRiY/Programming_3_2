...
function setup() {
    ...
    background('White');
    noLoop();
    img = loadImage("./bg.png");//background
    seasonImg = loadImage("./frame.png");
...
function draw() {
    ...
    fill("#2583fc");
    textSize(30);
    image(seasonImg,520 + img.width/9,img.height/5.6,seasonImg.width/10,img.height/35);
    text(season,520 + img.width/7,img.height/5);
} 
...
function main() {
	...
	socket.on("matrix",function(data){
        ...
        season = data[2];// ["Spring", "Summer", "Autumn", "Winter"]
        redraw();
    })
    fps = 10;
}
